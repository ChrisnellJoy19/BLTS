...
const UserEditResolution = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const resolution = location.state?.resolution || {};

  const [documentTitle, setDocumentTitle] = useState(resolution.documentTitle || "");
  const [documentType] = useState("Resolution");
  const [documentNumber, setDocumentNumber] = useState(resolution.documentNumber || "");
  const [governanceArea, setGovernanceArea] = useState(resolution.governanceArea || "");
  const [dateEnacted, setDateEnacted] = useState(resolution.dateEnacted ? resolution.dateEnacted.split("T")[0] : "");
  const [administrativeYear, setAdministrativeYear] = useState(resolution.administrativeYear || "");
  const [authors, setAuthors] = useState(resolution.authors || []);
  ...
  const [selectedFileName, setSelectedFileName] = useState(resolution.fileUrl ? "Existing file available" : "No file chosen");
  const [barangayId, setBarangayId] = useState(resolution.barangayId ? String(resolution.barangayId) : "");
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 p-6 bg-gradient-to-br from-[#889FB1] to-[#587D9D] text-black relative">
        ...
        <div className="max-w-4xl mx-auto mt-6 h-[70vh] overflow-y-auto bg-white p-6 rounded-lg shadow-lg">
          <h2 className="text-lg font-bold">Edit Resolution</h2>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={handleSubmit}>
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex items-center gap-2 w-full border p-2 rounded mt-2">
                ...
                >
                  {selectedFile || resolution.fileUrl ? "Change File" : "Choose File"}
                </button>
            ...
            <div className="md:col-span-2 flex justify-end gap-2">
              ...
              <button type="submit" className="bg-[#223645] text-white px-4 py-2 rounded hover:bg-[#1a2a35]">Update Resolution</button>
            </div>
          </form>
        </div>
      </main>
...
export default UserEditResolution;
